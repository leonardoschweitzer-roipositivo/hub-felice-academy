...
/* Questionário de qualificação do lead pós-compra da Recepção de Alta
   Performance. Parte das 8 perguntas de porte de clínica já usadas no
   Kit e no Gestão F4 (elas permitem inferir a capacidade de faturamento
   sem perguntar faturamento), com DUAS trocadas pelas que importam aqui:
   saíram `tratamentos` (mix de especialidades) e `tempo` (idade da
   clínica), entraram quem atende o telefone/WhatsApp e como o
   agendamento é feito — que é o que o curso mexe. */
const QUESTOES: { id: string; label: string; options: string[] }[] = [
  ...
  {
    id: 'atendimento',
    label: 'Quem atende o telefone e o WhatsApp da clínica hoje?',
    options: [
      'Eu mesmo(a)',
      'Uma recepcionista',
      'Duas ou mais pessoas',
      'Ninguém dedicado — quem estiver livre',
    ],
  },
  {
    id: 'agendamento',
    label: 'Como o agendamento é feito hoje?',
    options: [
      'Caderno ou agenda de papel',
      'Planilha',
      'Software de gestão',
  ...
  },
  {
    ...
    options: [
      'Padronizar a equipe',
      'Reduzir faltas e no-show',
      'Melhorar a experiência do paciente',
      'Aumentar os agendamentos',
      'Organizar a rotina da recepção',
    ],
  },
];

/** Monta a mensagem que o lead envia: origem do funil, contato e as
 *  respostas por extenso, para a conversa já começar qualificada.
...
function montarLinkWhatsapp(
  ...
  const linhas = [
    'Olá! Adquiri a Recepção de Alta Performance e quero agendar minha consultoria gratuita de 1 hora.',
    '',
    `*Origem:* ${ORIGEM_LABEL}`,
    `*Nome:* ${contato.nome}`,
...
export function ConsultoriaQuiz() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      const tracking = fireApplication('recepcao-alta-performance', 'post_purchase');
      const payload = {
        contato,
        qualificacao: answers,
        origem: 'recepcao-alta-performance/obrigado',
        tracking,
      };
      // barra final: o projeto usa trailingSlash, evita 308 no POST
      const res = await fetch('/produtos/recepcao-alta-performance/consultoria/api/lead/', {
        method: 'POST',
      ...
      // contato e respostas, então o agendamento chega à equipe mesmo se o
    ...
    <main className="cons-page">
      ...
      <div className="wrap cons-inner">
        <div className="cons-head reveal">
          <span className="eyebrow">Consultoria gratuita · Recepção de Alta Performance</span>
          <h1>Antes de agendar, me conta sobre a sua clínica</h1>
          <p className="cons-lead">
            São {QUESTOES.length} perguntas rápidas (menos de 2 minutos) para que o Dr. Sócrates
            chegue na sua consultoria já entendendo como a sua recepção funciona hoje — e o tempo da
            reunião renda o máximo.
          </p>
        </div>
        ...
        <form className="cons-form" onSubmit={submit}>
          ...
          <div className="cons-submit">
            ...
            >
              {submitting ? 'Abrindo o WhatsApp…' : 'Enviar e agendar minha consultoria'}{' '}
