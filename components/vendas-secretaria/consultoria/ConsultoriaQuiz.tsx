...
function montarLinkWhatsapp(
  ...
  const linhas = [
    'Olá! Adquiri o CRC de Alta Performance e quero agendar minha consultoria gratuita de 1 hora.',
    '',
    `*Origem:* ${ORIGEM_LABEL}`,
    `*Nome:* ${contato.nome}`,
...
export function ConsultoriaQuiz() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      const tracking = fireApplication('vendas-secretaria', 'post_purchase');
      const payload = {
        contato,
        qualificacao: answers,
        origem: 'vendas-secretaria/obrigado',
        tracking,
      };
      // barra final: o projeto usa trailingSlash, evita 308 no POST
      const res = await fetch('/produtos/vendas-secretaria/consultoria/api/lead/', {
        method: 'POST',
      ...
      <div className="wrap cons-inner">
        <div className="cons-head reveal">
          ...
          <p className="cons-lead">
            ...
            chegue na sua consultoria já entendendo como a sua recepção atende hoje — e o tempo da
